...
const QuantitativeTest = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const calculatedScore = quizData.reduce(
      (total, q) => total + (answers[q.id] === correctAnswers[q.id] ? 1 : 0),
      0
    );
    ...
    try {
      ...
      if (!response.ok) {
        setError("Failed to submit: " + result.message);
      }
    } catch (err) {
      console.error("Submission error:", err);
      setError("Could not submit at this time.");
    }
  };

  return (
    <div className="container quant-test">
      <div className="header-row">
        <div>
          <p className="category">General Aptitude Test</p>
          <h1>Quantitative Aptitude Assessment</h1>
          <p className="subtitle">
            Give this test in one go. You can change your answer before submitting.
          </p>
          {userData?.name && <p className="user-info">Candidate: {userData.name}</p>}
        </div>
      </div>
